...
class sum_node:
    ...
    def evaluate(self):
        ...
        if(self in value_dict):
            ...
            value_dict[self] = self.operand_1.evaluate() + self.operand_2.evaluate()
            return value_dict[self]
                    ...
                    for j in range(size[1]):
                        if type(operand.matrix.item(i,j)) == constant_node:
                            ...
                            if type(operand.matrix.item(i,j)) == variable_node:
                                m[i][j] = value_dict[operand.matrix.item(i,j)]
                            else:
                                m[i][j] = operand.matrix.item(i,j).evaluate()
                return m

            value_dict[self] = np.linalg.det(prepare_matrix(self.operand))
            return value_dict[self]
...
class solver_node:
    ...
    def evaluate(self):
        ...
        if self in value_dict:
            ...
            value_dict[self] = np.linalg.solve(self.operand_1.evaluate(),self.operand_2.evaluate())
            return value_dict[self]
    def differentiate(self,v,zero=None,one=None):
